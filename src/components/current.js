...
const Current = () => {
  ...
  return (
    <div className="flex flex-col items-center w-1/2 p-2">
      {currentWeather.name && (
        <>
          <div className="text-lg font-bold">{currentWeather.name}</div>
          <div className="text-red-600 text-lg p-2 m-2">{currentWeather.main.temp} &#8451;</div>
          <div className="text-red-600 p-2 m-2">{currentWeather.main.feels_like} &#8451;</div>
          <div className="text-red-600 font-bold p-2 m-2">{currentWeather.weather[0].main}</div>
          <div className="text-red-600 p-2 m-2">{currentWeather.weather[0].description}</div>
          ...
          <div className="text-red-600 p-2 m-2">
            {new Date(currentWeather.sys.sunrise * 1000).toISOString()}
          </div>
          <div className="text-red-600 p-2 m-2">
            {new Date(currentWeather.sys.sunset * 1000).toISOString()}
          </div>
